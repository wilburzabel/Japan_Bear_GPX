...
# 0. 页面配置
# ==========================================
st.set_page_config(page_title="熊出没地图 (最终版)", layout="wide", page_icon="🐻")
st.title("🐻 熊出没安全地图")
...
with col2:
    if uploaded_file:
        # 调试信息 (可以保留，确认数据读取正常)
        if points_count > 0:
            st.info(f"📍 GPX: {points_count} 点 | 🔎 粗筛: {len(candidates) if 'candidates' in locals() else 0} 条")
        ...
        if danger_list:
            ...
            if points_count > 0:
                st.success(f"🟢 安全 (检测范围: {buffer_radius_m}米)")
                st.caption("路线周边未发现记录。")
    else:
        st.info("👈 请在左侧上传 GPX 文件")
